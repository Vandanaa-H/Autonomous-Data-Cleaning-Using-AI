...
def generate_pdf_report(analysis_data, cleaning_report=None):
    ...
    try:
        ...
        # Title
        story.append(Paragraph("Comprehensive Data Quality Analysis & Cleaning Report", title_style))
        ...
        summary_data.append(['Original Size', f'{total_rows:,} rows × {total_columns} columns'])
        ...
        if cleaning_report:
            ...
            before_issues = len(cleaning_report.get('before_quality', {}).get('issues', []))
            after_issues = len(cleaning_report.get('after_quality', {}).get('issues', []))
            ...
            summary_data.extend([
                ...
                ['Issues Resolved', f'{before_issues - after_issues} out of {before_issues}'],
                ['Cleaning Success Rate', f'{((before_issues - after_issues) / before_issues * 100) if before_issues > 0 else 100:.1f}%']
            ])
        ...
        if analysis_data.get('issues'):
            story.append(Paragraph("Original Data Quality Issues", header_style))
            
            # Group issues by severity for better organization
            high_issues = [issue for issue in analysis_data['issues'] if issue.get('severity', 'medium') == 'high']
            medium_issues = [issue for issue in analysis_data['issues'] if issue.get('severity', 'medium') == 'medium']
            low_issues = [issue for issue in analysis_data['issues'] if issue.get('severity', 'medium') == 'low']
            
            issues_data = [['Column', 'Issue Type', 'Count', 'Percentage', 'Severity']]
            ...
            issues_table = Table(issues_data, colWidths=[1.5*inch, 1.8*inch, 0.8*inch, 1*inch, 1*inch])
        ...
        if cleaning_report and cleaning_report.get('steps_applied'):
            ...
            story.append(Paragraph("Applied Cleaning Operations", header_style))
            ...
            for i, step in enumerate(steps, 1):
                if isinstance(step, dict):
                    ...
                    if step.get('description'):
                        story.append(Paragraph(f"Description: {step['description']}", normal_style))
                    ...
                    if step.get('details'):
                        ...
                        if isinstance(details, list):
                            ...
                            story.append(Paragraph(f"• {details}", normal_style))
                else:
        ...
        if cleaning_report:
            story.append(Paragraph("Post-Cleaning Quality Assessment", header_style))
        ...
        story.append(Paragraph("Generated by Autonomous Data Cleaning Platform | Data Quality Management", footer_style))
...
def generate_word_report(analysis_data, cleaning_report=None):
    ...
    if not DOCX_AVAILABLE:
        st.error("Word document generation is not available. python-docx package is not installed.")
        return None
    ...
    try:
        ...
        # Title
        title = doc.add_heading('Comprehensive Data Quality Analysis & Cleaning Report', 0)
        ...
        summary.add_run(f'• Original Size: {total_rows:,} rows × {total_columns} columns\n')
        ...
        if cleaning_report:
            ...
            summary.add_run(f'• Quality Score Before Cleaning: {before_quality:.1f}%\n')
            summary.add_run(f'• Quality Score After Cleaning: {after_quality:.1f}%\n')
            summary.add_run(f'• Quality Improvement: {improvement:+.1f}%\n')
            summary.add_run(f'• Issues Resolved: {before_issues - after_issues} out of {before_issues}\n')
            summary.add_run(f'• Cleaning Success Rate: {((before_issues - after_issues) / before_issues * 100) if before_issues > 0 else 100:.1f}%\n')
        ...
        if analysis_data.get('issues'):
            ...
            for issue in high_issues + medium_issues + low_issues:
                ...
                row_cells[1].text = issue.get('type', 'Unknown').replace('_', ' ').title()
                ...
                row_cells[5].text = description[:100] + ('...' if len(description) > 100 else '')
        ...
        if cleaning_report and cleaning_report.get('steps_applied'):
            ...
            for i, step in enumerate(steps, 1):
                ...
                if isinstance(step, dict):
                    step_para.add_run(f"{step.get('step', 'Cleaning Operation')}").bold = True
                    step_para.add_run('\n')
        ...
        if cleaning_report:
            ...
            quality_para.add_run(f'• Initial Quality Score: {before_quality:.1f}%\n')
            quality_para.add_run(f'• Final Quality Score: {after_quality:.1f}%\n')
            quality_para.add_run(f'• Overall Improvement: {improvement:+.1f}%\n')
            quality_para.add_run(f'• Issues Successfully Resolved: {before_issues - after_issues}\n')
            ...
            if after_quality >= 95:
                assessment_para.add_run('Excellent - Data quality is now at optimal level for analysis and modeling.')
            elif after_quality >= 85:
                assessment_para.add_run('Good - Data quality is acceptable for most analytical purposes.')
            elif after_quality >= 70:
                assessment_para.add_run('Fair - Data quality has improved but may benefit from additional domain-specific cleaning.')
            else:
                assessment_para.add_run('Needs Attention - Consider additional data sources or manual review for critical applications.')
        ...
        rec_para.add_run('• Implement regular data quality monitoring for ongoing data ingestion\n')
        rec_para.add_run('• Consider implementing data validation rules at the source to prevent future quality issues\n')
        rec_para.add_run('• Establish data governance policies for consistent data handling\n')
        rec_para.add_run('• Regular automated quality checks should be scheduled\n')
        rec_para.add_run('• Document data transformation rules for reproducibility\n\n')
        ...
        rec_para.add_run('• Establish backup and recovery procedures for cleaned data\n')
        ...
        tech_para.add_run('• Statistical Outlier Identification (IQR Method)\n')
        ...
        footer_run = footer_para.add_run('Generated by AI-Powered Data Cleaning Platform | Professional Data Quality Management')
...
def clean_data(file_id):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Fetch original and cleaned data for display
            try:
                original_response = requests.get(f"{API_BASE_URL}/data/{file_id}/original", timeout=30)
                if original_response.status_code == 200:
                    original_data_json = original_response.json()
                    st.session_state.original_data = pd.DataFrame(original_data_json['data'])
                
                cleaned_response = requests.get(f"{API_BASE_URL}/data/{file_id}/cleaned", timeout=30)
                if cleaned_response.status_code == 200:
                    cleaned_data_json = cleaned_response.json()
                    st.session_state.cleaned_data = pd.DataFrame(cleaned_data_json['data'])
            except Exception as e:
...
def create_comprehensive_analysis_dashboard(analysis_data):
    ...
    with col1:
        st.markdown('<div class="chart-container scale-in">', unsafe_allow_html=True)
        st.markdown('<div class="chart-title">Issues by Severity Level</div>', unsafe_allow_html=True)
        ...
        if analysis_data.get('issues'):
            ...
            fig.update_layout(
                ...
                annotations=[dict(text='Issues', x=0.5, y=0.5, font_size=20, showarrow=False, font_color='#2c3e50')]
            )
    ...
    with col2:
            ...
            if missing_data:
                ...
                fig = go.Figure(data=[go.Bar(
                    ...
                    hovertemplate='<b>%{y}</b><br>Missing: %{x:.1f}%<br>Count: ' + 
                                missing_df['Missing_Count'].astype(str) + '<extra></extra>',
                    text=missing_df['Missing_Percentage'].round(1).astype(str) + '%',
                    textposition='auto'
                )])
                ...
                fig.update_layout(
                    ...
                    xaxis=dict(
                        title=dict(text="Missing Data Percentage (%)", font=dict(size=18)),
                        showgrid=True,
        ...
        st.markdown('</div>', unsafe_allow_html=True)    # Detailed Issues Report (Web-based, not downloadable)
    if analysis_data.get('issues'):
        ...
        with st.expander("Click to view detailed issues breakdown", expanded=False):
            ...
            for severity in ['high', 'medium', 'low']:
                ...
                if len(severity_issues) > 0:
                    ...
                    st.markdown(f"### {severity_label[severity]} Severity Issues ({len(severity_issues)})")
...
def create_comprehensive_cleaning_dashboard(cleaning_report):
    ...
    fig.add_trace(go.Bar(
        ...
        marker=dict(
            color=after_quality,
            colorscale=[[0, '#e74c3c'], [0.7, '#f39c12'], [0.85, '#27ae60'], [1, '#2ecc71']],
            cmin=0,
            cmax=100,
            line=dict(color='rgba(255,255,255,0.6)', width=2)
        ),
...
def create_complete_detailed_report(cleaning_report):
    ...
    st.markdown('<div class="section-header">Complete Data Cleaning Report</div>', unsafe_allow_html=True)
    ...
    if cleaning_report.get('before_quality', {}).get('issues'):
        ...
        for severity in ['high', 'medium', 'low']:
            severity_issues = issues_df[issues_df['severity'] == severity]
            if len(severity_issues) > 0:
                st.markdown(f"**{severity.upper()} Priority Issues ({len(severity_issues)}):**")
                for _, issue in severity_issues.iterrows():
                    st.markdown(f"- **{issue['column']}**: {issue['type'].replace('_', ' ').title()} ({issue['count']} instances, {issue['percentage']}%)")
    ...
    if cleaning_report.get('steps_applied'):
        ...
        for i, step in enumerate(steps, 1):
            if isinstance(step, dict):
                st.markdown(f"**{i}. {step.get('step', 'Cleaning Operation')}**")
                if step.get('description'):
    ...
    if cleaning_report.get('after_quality', {}).get('issues'):
        ...
        if remaining_issues:
            ...
            for _, issue in issues_df.iterrows():
                st.markdown(f"- **{issue['column']}**: {issue['type'].replace('_', ' ').title()} ({issue['count']} instances)")
        else:
    ...
    if after_quality >= 95:
        st.markdown("- **Excellent**: Data quality is now at an optimal level for analysis and modeling")
    elif after_quality >= 85:
        st.markdown("- **Good**: Data quality is acceptable, minor issues may remain but won't significantly impact analysis")
    elif after_quality >= 70:
        st.markdown("- **Fair**: Data quality has improved but may benefit from additional domain-specific cleaning")
    else:
        st.markdown("- **Needs Attention**: Consider additional data sources or manual review for critical applications")
    
    st.markdown("- Regular data quality monitoring is recommended for ongoing data ingestion")
    st.markdown("- Consider implementing data validation rules at the source to prevent future quality issues")
    ...
    with st.expander("Technical Details"):
        ...
        if cleaning_report.get('before_quality', {}).get('shape'):
            ...
            st.markdown(f"- Original dataset: {original_shape['rows']:,} rows × {original_shape['columns']} columns")
        
        if cleaning_report.get('after_quality', {}).get('shape'):
            final_shape = cleaning_report['after_quality']['shape']
            st.markdown(f"- Final dataset: {final_shape['rows']:,} rows × {final_shape['columns']} columns")
    ...
    with col2:
        ...
        if not available_formats:
            st.warning("⚠️ Report generation packages not available. Please install 'reportlab' for PDF reports and/or 'python-docx' for Word documents.")
            st.info("Run: `pip install reportlab python-docx` to enable report downloads")
        else:
            ...
            try:
                if report_format == "PDF Report" and REPORTLAB_AVAILABLE:
                    ...
                    pdf_content = generate_pdf_report(analysis_data, cleaning_report)
                    if pdf_content:
                        st.download_button(
                    ...
                    analysis_data = st.session_state.get('analysis_data', {})
                    word_content = generate_word_report(analysis_data, cleaning_report)
                    if word_content:
                        st.download_button(
        ...
        if st.session_state.current_step == 2:
            ...
            if st.session_state.show_complete_report:
                ...
                if st.session_state.cleaning_report:
                    create_complete_detailed_report(st.session_state.cleaning_report)
            else:
