...
class NLPProcessor:
    ...
    def detect_semantic_duplicates(self, df: pd.DataFrame, text_columns: List[str] = None, threshold: float = 0.85) -> List[DetectedIssue]:
        ...
        if not self.initialized or not self.embedding_model:
            logger.warning("Embedding model not available, skipping semantic duplicate detection")
            return issues
        ...
        for column in text_columns:
            ...
            try:
                ...
                for i in range(len(similarity_matrix)):
                    for j in range(i + 1, len(similarity_matrix)):
                        if similarity_matrix[i][j] > threshold:
                            ...
                            duplicate_pairs.append((text_values[i], text_values[j], similarity_matrix[i][j]))
                ...
                if semantic_duplicates > 0:
                    ...
                    logger.info(f"Found {semantic_duplicates} semantic duplicates in column '{column}'")
                
            except Exception as e:
                logger.error(f"Error detecting semantic duplicates in column '{column}': {e}")
    ...
    def detect_text_anomalies(self, df: pd.DataFrame, text_columns: List[str] = None) -> List[DetectedIssue]:
        ...
        for column in text_columns:
            ...
            if self.spacy_model:
                entity_issues = self._detect_entity_inconsistencies(df[column], column)
    ...
    def _detect_spelling_errors(self, series: pd.Series, column_name: str) -> List[DetectedIssue]:
        ...
        try:
            ...
            logger.error(f"Error detecting spelling errors in column '{column_name}': {e}")
    ...
    def _detect_entity_inconsistencies(self, series: pd.Series, column_name: str) -> List[DetectedIssue]:
        ...
        try:
            ...
            logger.error(f"Error detecting entity inconsistencies in column '{column_name}': {e}")
        ...
        try:
            ...
            for text in series.dropna().astype(str):
                ...
                try:
                    ...
                    if column_name.lower() in ['id', 'code', 'number']:  # These shouldn't have special chars
                        encoding_problems += 1
            ...
            logger.error(f"Error detecting encoding issues in column '{column_name}': {e}")
                    ...
                    if len(word) > 2:
                        ...
                        if suggestions and suggestions[0].term != word.lower():
                            ...
                            if suggestions[0].distance <= 1:
                                corrected_text = corrected_text.replace(word, suggestions[0].term)
            except Exception as e:
