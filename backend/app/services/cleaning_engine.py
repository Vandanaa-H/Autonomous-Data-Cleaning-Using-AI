...
class CleaningEngine:
    ...
    async def clean_dataset(
        ...
        try:
            ...
            for detector_name, detector in self.detectors.items():
                ...
                logger.info(f"{detector_name} detector found {len(issues)} issues")
            ...
            for issue in all_issues:
                ...
                strategies = self.strategy_selector.get_strategies_for_issue(issue, target_task)
                best_strategy = self.strategy_selector.select_best_strategy(
                    df, issue, strategies, target_task
                )
                
                # Apply cleaning strategy
                df, result = self.data_cleaner.apply_strategy(df, issue, best_strategy)
                ...
                logger.info(f"Applied {best_strategy.strategy_name} for {issue.issue_type}")
            ...
            logger.info(f"Cleaning completed for {file_id} in {processing_time:.2f} seconds")
            return report
    ...
    def _save_cleaned_data(self, file_id: str, df: pd.DataFrame, original_path: Path) -> Path:
        ...
        cleaned_file_path = settings.OUTPUT_DIR / f"{file_id}_cleaned{extension}"
